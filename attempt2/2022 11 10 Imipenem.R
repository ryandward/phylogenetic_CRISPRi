library(tidyverse)
library(data.table)
data <- fread("attempt2/20231110_imipenem.tsv") %>% melt(id.vars = c("Seconds", "Temp"), variable.name = "Well", value.name = "OD600")
...
ggplot(data, aes(x = Seconds, y = log10(OD600), group = interaction(Imipenem, Induced))) +
  ...
  labs(title = "E. coli library growth", x = "Seconds", y = "Growth (log OD600)",
       color = "Imipenem", linetype = "Induced", fill = "Imipenem") +
